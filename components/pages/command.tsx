...
const CommandCompo = () => {
  ...
  useEffect(() => {
    if (hasInitialized.current) {
      console.log('now hasInitialized')
      return
    }
    ...
    socket.on('change_lyrics', (data) => {
      if (data.stat === 0 && typeof window !== 'undefined') {
        setIsPlay('true')
        localStorage.setItem(IS_PLAY_STATE, 'true')
      } else if (data.stat === 2 && typeof window !== 'undefined') {
        setIsPlay('false')
        localStorage.setItem(IS_PLAY_STATE, 'false')
      } else if (isPlay === 'false' && data.stat === 1 && typeof window !== 'undefined') {
        setIsPlay('true')
        localStorage.setItem(IS_PLAY_STATE, 'true')
      }
    })
  ...
  if (setlist.length === 0) {
    ...
    if (isPlay === 'true') {
      return <ul>{playingOperation}</ul>
    } else {
      return <ul>{pauseOrperation}</ul>
    }
  }
}
